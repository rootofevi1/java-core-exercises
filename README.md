...
Десять консольных программ Александра, написанных во время обучения в «Школе 21» (`AP1_Jv_T01`). Проект показывает базовые навыки Java: обработку ввода, работу с числами, массивами, строками, файлами и объектами. Эти навыки использую как основу для автоматизации тестирования.
...
- При подготовке публикации проверены сборка и по одному сценарию запуска каждого из десяти модулей, включая содержимое выходного файла в `exercise7`.
- GitHub Actions собирает все десять модулей на Java 21. Автотестов в исходном проекте нет; успешная сборка не означает полного покрытия поведения.
- В первом модуле координаты вводятся по одной строке с десятичной точкой, например `0.0`; запись целого числа без точки не принимается.
- Среднее в `exercise4` вычисляется целочисленно; дробная часть отбрасывается.
- Рекурсивный Фибоначчи работает медленно для больших значений, даже внутри разрешённого диапазона до 46.
- В `exercise8` равные соседние числа нарушают строгое возрастание; индекс первого нарушения начинается с нуля.
- Обработка неверного ввода различается между модулями. Например, `exercise9` не перехватывает ошибку преобразования количества строк. Переполнения и все варианты завершения ввода не покрыты.

## Происхождение

Учебные решения Александра из `AP1_Jv_T01`. Для портфолио добавлены общая сборка и команды `run`, выбран JDK 21, путь выходного файла в седьмом модуле сделан относительно его каталога. Демонстрационный файл создан отдельно для публичного примера.

Школьные README, тексты заданий, комментарии с условиями и исходными тестовыми данными, служебные материалы и история учебного репозитория не включены.
